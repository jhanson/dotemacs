...
;; updates a create delegate with function to Ext.bind
(fset 'extbind
   [?\C-s ?c ?r ?e ?a ?t ?e ?\C-m ?\M-b left ?\C-\M-p ?\C-r ?f ?u ?n ?c ?t ?i ?o ?n ?\C-m ?E ?x ?t ?. ?b ?i ?n ?d ?\( ?\C-s ?\{ ?\C-m left ?\C-\M-n ?, ?\M-d ?\C-d ? ])
